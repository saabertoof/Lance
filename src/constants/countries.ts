...
export const countryOptions: CountryOption[] = [
  ...
  { code: 'BQ', name: 'Bonaire, Sint Eustatius and Saba', aliases: ['caribbean netherlands'] },
  ...
  { code: 'CC', name: 'Cocos Islands', aliases: ['keeling islands'] },
  { code: 'CO', name: 'Colombia' },
  { code: 'KM', name: 'Comoros' },
  { code: 'CG', name: 'Congo', aliases: ['republic of the congo'] },
  { code: 'CD', name: 'Congo (Democratic Republic)', aliases: ['democratic republic of the congo', 'drc'] },
  ...
  { code: 'VA', name: 'Holy See', aliases: ['vatican city'] },
  ...
  { code: 'LA', name: 'Lao People Democratic Republic', aliases: ['laos'] },
  { code: 'LV', name: 'Latvia' },
  ...
  { code: 'TR', name: 'Turkiye', aliases: ['turkey'] },
  ...
  { code: 'GB', name: 'United Kingdom', aliases: ['uk', 'great britain', 'england', 'scotland', 'wales'] },
  { code: 'US', name: 'United States', aliases: ['usa', 'us', 'america', 'united states of america'] },
...
for (const country of countryOptions) {
  ...
  countryLookup.set(normalizeCountryLookup(country.name), country.code);
  country.aliases?.forEach((alias) => {
    countryLookup.set(normalizeCountryLookup(alias), country.code);
  });
}
...
export function countryFlag(countryCode?: string | null) {
  ...
  const offset = 127397;
  return String.fromCodePoint(...code.split('').map((character) => character.charCodeAt(0) + offset));
}

export function countryNameForCode(countryCode?: string | null) {
  const code = normalizeCountryCode(countryCode);
  return code ? countryByCode.get(code)?.name ?? code : '';
}

export function countryOptionForCode(countryCode?: string | null) {
  const code = normalizeCountryCode(countryCode);
...
export function normalizeCountryCode(value?: string | null) {
  ...
  if (/^[A-Z]{2}$/.test(direct) && countryByCode.has(direct)) {
    return direct;
  }

  return countryLookup.get(normalizeCountryLookup(trimmed)) ?? '';
}
